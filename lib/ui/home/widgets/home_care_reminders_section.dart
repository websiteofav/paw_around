...
class _HomeCareRemindersSectionState extends State<HomeCareRemindersSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final grooming = widget.pet.groomingSettings;
    if (grooming != null &&
        grooming.hasReminder &&
        !_dismissed.contains('grooming')) {
      final days = grooming.daysUntilDue ?? 0;
      final itemName = grooming.groomingTypes.isNotEmpty
          ? grooming.groomingTypes.first
          : AppStrings.groomingSession;
      cards.add(_CareReminderCard(
        title: AppStrings.groomingDue,
        itemName: itemName,
        daysText: days < 0
            ? 'Overdue by ${days.abs()} days'
            : 'Due in $days ${days == 1 ? 'day' : 'days'}',
        frequencyLabel: grooming.frequency.displayName,
        bgColor: AppColors.quickActionGrooming,
        actionLabel: AppStrings.addGrooming,
